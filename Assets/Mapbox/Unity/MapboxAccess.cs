...
namespace Mapbox.Unity {
		...
		public void SetConfiguration(MapboxConfiguration configuration, bool throwExecptions = true) {
			...
			if (null == configuration || string.IsNullOrEmpty(configuration.AccessToken)) {
				...
				TokenValidator.Retrieve(
					configuration.GetMapsSkuToken, configuration.AccessToken, (response) =>
					{
...
#if !WINDOWS_UWP
			MapboxConfiguration test = JsonUtility.FromJson<MapboxConfiguration>(ConfigurationJSON);
	...
	public class MapboxConfiguration {
		...
		[NonSerialized] private MapboxAccounts mapboxAccounts = new();
