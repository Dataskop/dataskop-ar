...
namespace Mapbox.Unity.Map {
	...
	public class AbstractMap : MonoBehaviour, IMap {
		...
		public virtual void UpdateMap(Vector2d latLon, float zoom) {
			...
			Vector2d centerLatitudeLongitude = latLon;
			...
			zDelta = zDelta > 0 ? Mathd.Min(zDelta, Constants.LongitudeMax)
				: Mathd.Max(zDelta, -Constants.LongitudeMax);
		...
		public bool IsAccessTokenValid
		{
			get
			{
				...
				try {
					MapboxAccess accessTokenCheck = MapboxAccess.Instance;

					if (MapboxAccess.Instance.Configuration == null ||
					    string.IsNullOrEmpty(MapboxAccess.Instance.Configuration.AccessToken)) {
						return false;
					}
		...
		public void DestroyTileProvider() {
			AbstractTileProvider tileProvider = TileProvider ?? gameObject.GetComponent<AbstractTileProvider>();
		...
		protected virtual void TileProvider_OnTileAdded(UnwrappedTileId tileId) {
			UnityTile tile = _mapVisualizer.LoadTile(tileId);
		...
		private void ApplySnapWorldToZero(UnityTile referenceTile) {
			if (_options.placementOptions.snapMapToZero) {
				float h = referenceTile.QueryHeightData(.5f, .5f);
		...
		protected virtual void InitializeMap(MapOptions options) {
			...
			_options.locationOptions.PropertyHasChanged += (object sender, EventArgs eventArgs) => { UpdateMap(); };
					...
					case MapExtentType.CameraBounds:
					{
						if (TileProvider != null) {
							if (!(TileProvider is QuadTreeTileProvider)) {
								TileProvider.gameObject.Destroy();
								GameObject go = new(tileProviderName);
		...
		private void TriggerTileRedrawForExtent(ExtentArgs currentExtent) {
			Dictionary<UnwrappedTileId, UnityTile> _activeTiles = _mapVisualizer.ActiveTiles;
			...
			foreach (KeyValuePair<UnwrappedTileId, UnityTile> item in _activeTiles) {
			...
			if (tilesToProcess.Count > 0) {
				...
				foreach (UnwrappedTileId t2r in tilesToProcess) {
					TileProvider_OnTileRemoved(t2r);
				}
			}

			foreach (KeyValuePair<UnwrappedTileId, UnityTile> tile in _activeTiles) {
			...
			foreach (UnwrappedTileId tile in _currentExtent) {
			...
			if (tilesToProcess.Count > 0) {
				...
				foreach (UnwrappedTileId tileId in tilesToProcess) {
		...
		private void OnVectorDataSubLayerRemoved(object sender, EventArgs eventArgs) {
			...
			if (layerUpdateArgs.visualizer != null) {
				_mapVisualizer.RemoveTilesFromLayer(
					(VectorTileFactory)layerUpdateArgs.factory, layerUpdateArgs.visualizer
				);
			}
		...
		protected virtual float QueryElevationAtInternal(Vector2d latlong, out float tileScale) {
			Vector2d _meters = Conversions.LatLonToMeters(latlong.x, latlong.y);
			UnityTile tile;
			...
			if (foundTile) {
				...
				RectD _rect = tile.Rect;
				return tile.QueryHeightData(
					(float)((_meters - _rect.Min).x / _rect.Size.x), (float)((_meters.y - _rect.Max.y) / _rect.Size.y)
				);
			}
			else {
				tileScale = 1f;
			...
			if (queryHeight) {
				...
				if (MapVisualizer.ActiveTiles.TryGetValue(
					...
					if (tile != null) {
						...
						Vector3 localPos = tile.gameObject.transform.InverseTransformPoint(worldPos);
						localPos.y = height;
		...
		public virtual void SetExtent(MapExtentType extentType, ExtentOptions extentOptions = null) {
			...
			if (extentOptions != null) {
				ExtentOptions currentOptions = _options.extentOptions.GetTileProviderOptions();
