...
namespace Mapbox.Unity.Map.TileProviders {
	...
	public class QuadTreeTileProvider : AbstractTileProvider {
		...
		public UnwrappedTileId WebMercatorToTileId(Vector2d webMerc, int zoom) {
			...
			double dblX = webMerc.x / Utils.Constants.WebMercMax;
			double dblY = webMerc.y / Utils.Constants.WebMercMax;
		...
		private Vector2dBounds getcurrentViewPortWebMerc(bool useGroundPlane = true) {
			...
			Vector2d hitPntSWGeoPos = new(minLat, minLong);
			Vector2d hitPntNEGeoPos = new(maxLat, maxLong);
			Vector2dBounds tileBounds = new(
				Conversions.LatLonToMeters(hitPntSWGeoPos), Conversions.LatLonToMeters(hitPntNEGeoPos)
			); // Bounds debugging.
#if UNITY_EDITOR
