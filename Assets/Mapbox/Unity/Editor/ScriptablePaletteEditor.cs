...
public class ScriptablePaletteEditor : Editor {
	...
	public override void OnInspectorGUI() {
		...
		GUIStyle wrapTextStyle = new();
		wrapTextStyle.wordWrap = true;
		wrapTextStyle.normal.textColor = new Color(
			_HELPER_TEXT_COLOR_VAL, _HELPER_TEXT_COLOR_VAL, _HELPER_TEXT_COLOR_VAL, 1.0f
		);
		...
		GUIContent keyColorContent = new("Key color", _KEY_COLOR_INFO);
		sp.m_keyColor = EditorGUILayout.ColorField(keyColorContent, sp.m_keyColor);

		EditorGUILayout.Space();

		GUIContent numColorContent = new("Palette Size", _NUM_COLOR_INFO);
		sp.m_numColors = EditorGUILayout.IntSlider(numColorContent, sp.m_numColors, _NUM_COLORS_MIN, _NUM_COLORS_MAX);

		GUIContent hueRangeContent = new("Hue Range", _HUE_RANGE_INFO);
		GUIContent satRangeContent = new("Saturation Range", _SAT_RANGE_INFO);
		GUIContent valRangeContent = new("Value Range", _VAL_RANGE_INFO);
		...
		EditorGUILayout.LabelField(
			"Layer colors can be overriden by enabling any of the overrides below.", wrapTextStyle
		);
		EditorGUILayout.LabelField(
			"If a layer's color is overridden, it will be set directly from the defined override color.", wrapTextStyle
		);
