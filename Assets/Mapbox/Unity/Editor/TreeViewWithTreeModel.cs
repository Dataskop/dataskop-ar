	...
	internal class TreeViewWithTreeModel<T> : TreeView where T : TreeElement {
		...
		protected override TreeViewItem BuildRoot() {
			...
			return new TreeViewItem<T>(
				m_TreeModel.root.id, depthForHiddenRoot, m_TreeModel.root.name, m_TreeModel.root
			);
		}
		...
		protected virtual void SortSearchResult(List<TreeViewItem> rows) {
			rows.Sort(
				(x, y) => EditorUtility.NaturalCompare(x.displayName, y.displayName)
				...
				case DragAndDropPosition.BetweenItems:
				{
					...
					if (args.performDrop && validDrag) {
						...
						OnDropDraggedElementsAtIndex(
							draggedRows, parentData, args.insertAtIndex == -1 ? 0 : args.insertAtIndex
						);
					}
