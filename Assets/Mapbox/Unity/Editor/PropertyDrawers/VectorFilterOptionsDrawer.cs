namespace Mapbox.Editor {
	...
	public class VectorFilterOptionsDrawer : PropertyDrawer {
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
			...
			if (showFilters) {
				SerializedProperty propertyFilters = property.FindPropertyRelative("filters");
		...
		private void DrawLayerFilter(SerializedProperty originalProperty, SerializedProperty propertyFilters, int index,
			...
			SerializedProperty property = propertyFilters.GetArrayElementAtIndex(index);

			SerializedProperty filterOperatorProp = property.FindPropertyRelative("filterOperator");
			...
			string selectedLayerName = originalProperty.FindPropertyRelative("_selectedLayerName").stringValue;

			DrawPropertyDropDown(originalProperty, property);

			EditorGUI.BeginChangeCheck();
			filterOperatorProp.enumValueIndex = EditorGUILayout.Popup(
				filterOperatorProp.enumValueIndex, filterOperatorProp.enumDisplayNames, GUILayout.MaxWidth(150)
			);
		...
		private void DrawPropertyDropDown(SerializedProperty originalProperty, SerializedProperty filterProperty) {
			...
			string propertyString = filterProperty.FindPropertyRelative("Key").stringValue;
		...
		private void DrawWarningMessage() {
			...
			EditorGUILayout.LabelField(
				new GUIContent(), new GUIContent("No properties"), labelStyle, new GUILayoutOption[] {
