...
namespace Mapbox.Unity.MeshGeneration.Modifiers {
	...
	public class MergedModifierStack : ModifierStackBase {
		...
		private Dictionary<UnityTile, int> _cacheVertexCount = new();
		private Dictionary<UnityTile, List<MeshData>> _cached = new();
		private Dictionary<UnityTile, int> _buildingCount = new();

		private Dictionary<UnityTile, List<VectorEntity>> _activeObjects = new();
		...
		public GameObject End(UnityTile tile, GameObject parent, string name = "") {
			...
			if (_cached.ContainsKey(tile)) {
				...
				for (int i = 0; i < _counter; i++) {
					...
					int st = _tempMeshData.Vertices.Count;
		...
		public override void Clear() {
			foreach (VectorEntity vectorEntity in _pool.GetQueue()) {
			...
			foreach (KeyValuePair<UnityTile, List<VectorEntity>> tileTuple in _activeObjects) {
				foreach (VectorEntity vectorEntity in tileTuple.Value) {
					if (vectorEntity.Mesh != null) {
						vectorEntity.Mesh.Destroy(true);
