namespace Mapbox.Unity.MeshGeneration.Modifiers {
	...
	public class TextureSideWallModifier : MeshModifier {
		...
		public override void Initialize() {
			...
			foreach (AtlasEntity atlasEntity in _options.atlasInfo.Textures) {
		...
		public override void Run(VectorFeatureUnity feature, MeshData md, UnityTile tile = null) {
			...
			if (_options.extrusionGeometryType != ExtrusionGeometryType.RoofOnly) {
				...
				for (int i = 0; i < md.Edges.Count; i += 2) {
					Vector3 v1 = md.Vertices[md.Edges[i]];
					Vector3 v2 = md.Vertices[md.Edges[i + 1]];
		...
		private void CreateWall(MeshData md) {
			...
			wallNormal = new Vector3(
				-(wallSegmentFirstVertex.z - wallSegmentSecondVertex.z), 0,
		...
		private void TopFloor(MeshData md, float leftOver) {
			...
			md.Vertices.Add(
				new Vector3(
					wallSegmentFirstVertex.x, wallSegmentFirstVertex.y - leftOver,
					wallSegmentFirstVertex.z
				)
			);
			md.Vertices.Add(
				new Vector3(
					wallSegmentSecondVertex.x, wallSegmentSecondVertex.y - leftOver,
					wallSegmentSecondVertex.z
				)
			);
			md.Vertices.Add(
				new Vector3(
					wallSegmentFirstVertex.x, wallSegmentFirstVertex.y - leftOver - finalTopHeight,
					wallSegmentFirstVertex.z
				)
			);
			md.Vertices.Add(
		...
		private void FirstFloor(MeshData md, float hf) {
			md.Vertices.Add(
				new Vector3(
					wallSegmentFirstVertex.x, wallSegmentFirstVertex.y - hf + finalFirstHeight,
					wallSegmentFirstVertex.z
				)
			);
			md.Vertices.Add(
				new Vector3(
					wallSegmentSecondVertex.x, wallSegmentSecondVertex.y - hf + finalFirstHeight,
					wallSegmentSecondVertex.z
				)
			);
			md.Vertices.Add(
				new Vector3(
					wallSegmentFirstVertex.x, wallSegmentFirstVertex.y - hf,
					wallSegmentFirstVertex.z
				)
			);
			md.Vertices.Add(
				new Vector3(
					wallSegmentSecondVertex.x, wallSegmentSecondVertex.y - hf,
					wallSegmentSecondVertex.z
				)
			);
		...
		private void GenerateRoofMesh(MeshData md, float minHeight, float maxHeight) {
			...
			switch (_options.extrusionType) {
				...
				case ExtrusionType.PropertyHeight:
					for (int i = 0; i < _counter; i++) {
						md.Vertices[i] = new Vector3(
							md.Vertices[i].x, md.Vertices[i].y + maxHeight,
							md.Vertices[i].z
						);
					}
				...
				{
					MinMaxPair minmax = MinMaxPair.GetMinMaxHeight(md.Vertices);
				...
				case ExtrusionType.RangeHeight:
					if (feature.Properties.ContainsKey(_options.propertyName)) {
						if (_options.minimumHeight > _options.maximumHeight) {
							...
							float temp = _options.minimumHeight;
							_options.minimumHeight = _options.maximumHeight;
							_options.maximumHeight = temp;
						}

						float featureHeight = Convert.ToSingle(feature.Properties[_options.propertyName]);
						maxHeight = Math.Min(Math.Max(_options.minimumHeight, featureHeight), _options.maximumHeight);

						if (feature.Properties.ContainsKey("min_height")) {
							float featureMinHeight = Convert.ToSingle(feature.Properties["min_height"]);
