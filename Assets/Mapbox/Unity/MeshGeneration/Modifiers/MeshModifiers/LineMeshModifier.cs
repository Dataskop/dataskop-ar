...
namespace Mapbox.Unity.MeshGeneration.Modifiers {
	...
	public class LineMeshModifier : MeshModifier {
		...
		private void ExtrudeLine(VectorFeatureUnity feature, MeshData md) {
			...
			foreach (List<Vector3> roadSegment in feature.Points) {
				...
				int roadSegmentCount = roadSegment.Count;
				...
				for (int i = 0; i < roadSegmentCount; i++) {
					...
					Vector3 joinNormal = (_prevNormal + _nextNormal).normalized;
					...
					float cosHalfAngle = joinNormal.x * _nextNormal.x + joinNormal.z * _nextNormal.z;
					float miterLength = cosHalfAngle != 0 ? 1 / cosHalfAngle : float.PositiveInfinity;
					bool isSharpCorner = cosHalfAngle < _cosHalfSharpCorner &&
					                     _prevVertex != Constants.Math.Vector3Unused &&
					                     _nextVertex != Constants.Math.Vector3Unused;

					if (isSharpCorner && i > 0) {
						float prevSegmentLength = Vector3.Distance(_currentVertex, _prevVertex);

						if (prevSegmentLength > 2 * _sharpCornerOffset) {
							Vector3 dir = _currentVertex - _prevVertex;
							Vector3 newPrevVertex = _currentVertex - dir * (_sharpCornerOffset / prevSegmentLength);
					...
					if (currentJoin == JoinType.Miter) {
						...
						bool lineTurnsLeft = _prevNormal.x * _nextNormal.z - _prevNormal.z * _nextNormal.x > 0;
						float offset = (float)-Math.Sqrt(miterLength * miterLength - 1);
						...
						if (!_startOfLine) {
							AddCurrentVertex(
								_currentVertex, _distance, _prevNormal, md, _cornerOffsetA, _cornerOffsetB
							);
						}
						...
						if (currentJoin == JoinType.Fakeround) {
							...
							float n = Mathf.Floor((0.5f - (cosHalfAngle - 0.5f)) * 8);
						...
						if (_nextVertex != Constants.Math.Vector3Unused) {
							AddCurrentVertex(
								_currentVertex, _distance, _nextNormal, md, -_cornerOffsetA, -_cornerOffsetB
							);
						}
					}
					...
					if (isSharpCorner && i < roadSegmentCount - 1) {
						float nextSegmentLength = Vector3.Distance(_currentVertex, _nextVertex);
		...
		private void AddPieSliceVertex(Vector3 vertexPosition, float dist, Vector3 normal, bool lineTurnsLeft,
			MeshData md) {
			int triIndexStart = md.Vertices.Count;
			Vector3 extrude = normal * (lineTurnsLeft ? -1 : 1);
		...
		private void AddCurrentVertex(Vector3 vertexPosition, float dist, Vector3 normal, MeshData md,
			...
			int triIndexStart = md.Vertices.Count;
			Vector3 extrude = normal;

			if (endLeft != 0) {
				extrude -= normal.Perpendicular() * endLeft;
			}

			Vector3 vert = vertexPosition + extrude * _scaledWidth;
