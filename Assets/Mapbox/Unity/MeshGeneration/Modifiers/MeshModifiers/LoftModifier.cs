	...
	public class LoftModifier : MeshModifier {
		...
		public override void Initialize() {
			...
			for (int i = 0; i < _sliceCount - 1; i++) {
				_sliceUvs[i + 1] = new Vector2(
					0, _sliceUvs[i].y + (_slice[i + 1] - _slice[i]).magnitude / _sliceTotalMagnitude
				);
			}
		}
		...
		public override void Run(VectorFeatureUnity feature, MeshData md, UnityTile tile = null) {
			...
			foreach (List<Vector3> roadSegment in feature.Points) {
				...
				List<Vector3> vl = new(_sliceCount * _counter);
				List<Vector3> edges = new(_counter);
				int co = 0;

				for (int j = 0; j < _counter; j++) {
					Vector3 current = Constants.Math.Vector3Zero;
					...
					Quaternion q = Quaternion.LookRotation(dirCurrent);

					co = _slice.Count;

					for (int i = 0; i < co; i++) {
						Vector3 p = q * _slice[i];
						vl.Add(p + current);
				...
				for (int i = 0; i < _counter - 1; i++) {
					for (int j = 0; j < _sliceCount - 1; j++) {
						int ind = i * _sliceCount + j;
						Vector3 ed = vl[ind + _sliceCount] - vl[ind];
						edMag = ed.magnitude;
				...
				if (_closeEdges && edges.Count > 2) {
					...
					Data flatData = EarcutLibrary.Flatten(
