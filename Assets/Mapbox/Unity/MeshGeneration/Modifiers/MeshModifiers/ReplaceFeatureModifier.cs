namespace Mapbox.Unity.MeshGeneration.Modifiers {
	...
	public class ReplaceFeatureModifier : GameObjectModifier, IReplacementCriteria {
		...
		public override void Initialize() {
			...
			foreach (string loc in _prefabLocations) {
		...
		public override void FeaturePreProcess(VectorFeatureUnity feature) {
			...
			foreach (string point in _prefabLocations) {
				try {
					index++;
					Vector2d coord = Conversions.StringToLatLon(point);
		...
		public bool ShouldReplaceFeature(VectorFeatureUnity feature) {
			...
			foreach (string blockedId in _explicitlyBlockedFeatureIds) {
			...
			foreach (string point in _prefabLocations) {
				try {
					...
					if (_featureId[index] != null) {
						foreach (string featureId in _featureId[index]) {
							Vector2d latlngVector = Conversions.StringToLatLon(point);
							Vector2d from = Conversions.LatLonToMeters(latlngVector.x, latlngVector.y);
							Vector2d to = new(
		...
		private void SpawnPrefab(VectorEntity ve, UnityTile tile, Vector2d latLong) {
			...
			ulong featureId = ve.Feature.Data.Id;
		...
		private bool ShouldSpawnFeature(VectorFeatureUnity feature, out Vector2d latLong) {
			...
			foreach (Vector2d point in _latLonToSpawn) {
		...
		public override void OnPoolItem(VectorEntity vectorEntity) {
			...
			ulong featureId = vectorEntity.Feature.Data.Id;

			if (!_objects.ContainsKey(featureId)) {
				return;
			}

			GameObject go = _objects[featureId];
