namespace Mapbox.Unity.MeshGeneration.Modifiers {
	...
	public class ChamferHeightModifier : MeshModifier {
		...
		public override void Run(VectorFeatureUnity feature, MeshData md, UnityTile tile = null) {
			...
			float max = md.Vertices[0].y;
			float min = md.Vertices[0].y;
			...
			int originalVertexCount = md.Vertices.Count;
		...
		private void Sides(VectorFeatureUnity feature, MeshData meshData, float hf, int originalVertexCount) {
			...
			List<int> wallTri = new();
			List<Vector2> wallUv = new();
			meshData.Vertices.Add(
				new Vector3(
					meshData.Vertices[originalVertexCount - 1].x, meshData.Vertices[originalVertexCount - 1].y - hf,
		...
		private static void FlattenTops(MeshData meshData, float minHeight, ref float hf, ref float max,
			ref float min) {
			...
			for (int i = 0; i < meshData.Vertices.Count; i++) {
				meshData.Vertices[i] = new Vector3(
					meshData.Vertices[i].x, max + minHeight + hf, meshData.Vertices[i].z
				);
			}
		...
		public void Chamfer(VectorFeatureUnity feature, MeshData md, UnityTile tile = null) {
			...
			for (int i = 0; i < feature.Points.Count; i++) {
				int count = feature.Points[i].Count;
				int cst = newVertices.Count;
				...
				for (int j = 0; j < count; j++) {
					...
					float d = Vector3.Distance(poi, pij2);

					if (d > 10 * _offset) {
						poi = new Vector3(
							md.Vertices[current].x + (poi - (-v1 - v2)).normalized.x, 0,
		...
		private List<Vector3> GetEnlargedPolygon(List<Vector3> old_points, float offset) {
			List<Vector3> enlarged_points = new();
