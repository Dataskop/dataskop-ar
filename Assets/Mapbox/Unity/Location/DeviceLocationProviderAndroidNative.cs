namespace Mapbox.Unity.Location {
	...
	public class DeviceLocationProviderAndroidNative : AbstractLocationProvider, IDisposable {
		...
		protected virtual void Awake() {
			...
			_waitUpdateTime = _updateTimeInMilliSeconds < 500 ? new WaitForSeconds(0.5f)
		...
		private void getGpsInstance(bool showToastMessages = false) {
			...
			using (AndroidJavaClass androidGps = new("com.mapbox.android.unity.AndroidGps")) {
				if (null == androidGps) {
				...
				_activityContext.Call(
					"runOnUiThread",
					new AndroidJavaRunnable(() => { _gpsInstance.Call("showMessage", "starting location listeners"); })
				);
		...
		private void getSensorInstance() {
			...
			using (AndroidJavaClass androidSensors = new("com.mapbox.android.unity.AndroidSensors")) {
		...
		private void populateCurrentLocation(AndroidJavaObject location) {
			...
			Vector2d newLatLng = new(lat, lng);
