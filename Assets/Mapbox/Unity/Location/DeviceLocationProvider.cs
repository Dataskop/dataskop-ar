	...
	public class DeviceLocationProvider : AbstractLocationProvider {
		...
		private IEnumerator PollLocationRoutine() {
			...
			while (true) {
				...
				double timestamp = lastData.timestamp;
				...
				if (_currentLocation.IsLocationUpdated) {
					if (_lastPositions.Count > 0) {
						...
						CheapRuler cheapRuler = new(_currentLocation.LatitudeLongitude.x, CheapRulerUnits.Meters);
						...
						double distance = cheapRuler.Distance(
							...
							new double[] {
								_lastPositions[0].y, _lastPositions[0].x
							}
						);
				...
				if (_lastPositions.Count < _maxLastPositions) {
					...
					CheapRuler cheapRuler = new(newestPos.x, CheapRulerUnits.Meters);
					// distance between last and first position in our buffer
					double distance = cheapRuler.Distance(
						new double[] {
							newestPos.y, newestPos.x
						},
						new double[] {
							oldestPos.y, oldestPos.x
						}
					);
