	...
	PRIMARY KEY(
			...
			foreach (string cmd in cmds) {
				try {
					...
					if (ex.Result != SQLite3.Result.Row) {
						Debug.LogErrorFormat("{0}: {1}", cmd, ex);
		...
		public void Add(string tilesetName, CanonicalTileId tileId, CacheItem item, bool forceInsert = false) {
			...
			try {
				...
				if (1 != rowsAffected) {
					throw new Exception(
						string.Format(
							"tile [{0} / {1}] was not inserted, rows affected:{2}", tilesetName, tileId, rowsAffected
						)
					);
				}
			}
		...
		private int insertTileset(string tilesetName) {
			try {
				...
				if (1 != rowsAffected) {
					throw new Exception(
						string.Format("tileset [{0}] was not inserted, rows affected:{1}", tilesetName, rowsAffected)
					);
				}
