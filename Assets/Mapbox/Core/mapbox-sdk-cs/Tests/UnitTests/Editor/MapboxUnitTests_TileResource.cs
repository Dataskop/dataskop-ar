...
namespace Mapbox.MapboxSdkCs.UnitTest {
	...
	public class TileResourceTest {
		...
		public void GetUrlRaster() {
			TileResource res1 = TileResource.MakeRaster(_tileId, null);
			Assert.AreEqual(
				_api + "styles/v1/mapbox/satellite-v9/tiles/0/0/0", res1.GetUrl().Split("?".ToCharArray())[0]
			);

			TileResource res2 = TileResource.MakeRaster(_tileId, "mapbox://styles/mapbox/basic-v9");
		...
		public void GetUrlClassicRaster() {
			TileResource res1 = TileResource.MakeClassicRaster(_tileId, null);
			Assert.AreEqual(_api + "v4/mapbox.satellite/0/0/0.png", res1.GetUrl().Split("?".ToCharArray())[0]);

			TileResource res2 = TileResource.MakeClassicRaster(_tileId, "foobar");
			Assert.AreEqual(_api + "v4/foobar/0/0/0.png", res2.GetUrl().Split("?".ToCharArray())[0]);

			TileResource res3 = TileResource.MakeClassicRaster(_tileId, "test");
			Assert.AreEqual(_api + "v4/test/0/0/0.png", res3.GetUrl().Split("?".ToCharArray())[0]);
		}

		[Test]
		public void GetUrlVector() {
			TileResource res1 = TileResource.MakeVector(_tileId, null);
			Assert.AreEqual(
				_api + "v4/mapbox.mapbox-streets-v7/0/0/0.vector.pbf", res1.GetUrl().Split("?".ToCharArray())[0]
			);

			TileResource res2 = TileResource.MakeVector(_tileId, "foobar");
			Assert.AreEqual(_api + "v4/foobar/0/0/0.vector.pbf", res2.GetUrl().Split("?".ToCharArray())[0]);

			TileResource res3 = TileResource.MakeVector(_tileId, "test");
