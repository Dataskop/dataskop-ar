namespace Mapbox.MapboxSdkCs.UnitTest {
	...
	internal class SQLiteCacheTest {
		...
		public void Init() {
			...
			Vector2d southWest = new(48.2174, 16.3662);
			Vector2d northEast = new(48.2310, 16.3877);
			Vector2dBounds bounds = new(southWest, northEast);
		...
		public void InsertSameTileNoOverwrite() {
			...
			Assert.AreEqual(
				1, _cache.TileCount(TS_NO_OVERWRITE), "tileset {0}: unexpected number of tiles", TS_NO_OVERWRITE
			);
		}
		...
		public void VerifyTilesFromConcurrentInsert() {
			...
			string[] tilesetNames = new string[] {
				TS_CONCURRENT1, TS_CONCURRENT2, TS_CONCURRENT3, TS_CONCURRENT4
			};

			foreach (string tilesetName in tilesetNames) {
				Assert.AreEqual(
					_tileIds.Count, _cache.TileCount(tilesetName),
		...
		public void Prune() {
			string methodName = _className + "." + new StackFrame().GetMethod().Name;
			HashSet<CanonicalTileId> tileIds = new();
			...
			logTime(methodName, elapsed);
			Assert.AreEqual(
				_maxCacheTileCount, _cache.TileCount(TS_PRUNE), _cache.PruneCacheDelta,
