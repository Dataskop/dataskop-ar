...
namespace Mapbox.MapboxSdkCs.UnitTest {
	...
	internal class MapMatcherTest {
		...
		public IEnumerator AsSimpleAsPossible() {
			...
			mapMatcher.Match(
				resource,
				(MapMatchingResponse response) => { matchingResponse = response; }
			);
		...
		public IEnumerator Profiles() {
			...
			Assert.GreaterOrEqual(
				matchingResponse.Matchings[0].Duration, 300, "'mapbox/walking' duration [{0}] less than expected",
			...
			Assert.GreaterOrEqual(
				matchingResponse.Matchings[0].Duration, 100, "'mapbox/cycling' duration less than expected"
			);
			...
			Assert.GreaterOrEqual(
				matchingResponse.Matchings[0].Duration, 100, "'driving-traffic' duration less than expected"
			);
		...
		public IEnumerator NoSegment() {
			...
			Assert.AreEqual(
				"Could not find a matching segment for input coordinates", matchingResponse.Message,
		...
		public IEnumerator AlternativesWithSteps() {
			...
			Assert.AreEqual(
				3, matchingResponse.Matchings[0].Legs[0].Steps[0].Intersections.Count, "Wrong number of intersections"
			);
		}
		...
		public IEnumerator OverviewSimplified() {
			...
			Assert.GreaterOrEqual(
				matchingResponse.Matchings[0].Geometry.Count, 14, "Wrong number of vertices in match geometry"
			);
		}
		...
		public IEnumerator OverviewFull() {
			...
			Assert.GreaterOrEqual(
				matchingResponse.Matchings[0].Geometry.Count, 20, "Wrong number of vertices in match geometry"
			);
		}
		...
		public IEnumerator InvalidCoordinate() {
			...
			Assert.AreEqual(
				"Coordinate is invalid: 32.71204,-117.17283", matchingResponse.Message,
