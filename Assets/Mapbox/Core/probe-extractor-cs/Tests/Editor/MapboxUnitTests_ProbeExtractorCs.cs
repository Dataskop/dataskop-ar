...
namespace Mapbox.ProbeExtractorCs.UnitTest {
	...
	internal class ProbeExtractorCsTest {
		...
		public void ExtractProbes() {
			...
			ProbeExtractorOptions options = new() {
			...
			ProbeExtractor extractor = new(ruler, options);
		...
		private List<TracePoint> loadTraceFixture(string fixtureName) {
			...
			List<TracePoint> trace = new();

			using (StringReader sr = new(fixtureAsset.text)) {
				// skip header
				sr.ReadLine();
				string line;
		...
		private string probesToGeojson(List<Probe> probes) {
			...
			sb.Append(
				string.Join(
					",",
					probes.Select(
						p => string.Format(CultureInfo.InvariantCulture, "[{0},{1}]", p.Longitude, p.Latitude)
					).ToArray()
				)
			);
