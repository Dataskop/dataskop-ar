	...
	public class ProbeExtractor {
		...
		public List<Probe> ExtractProbes(List<TracePoint> trace) {
			...
			if (probes.Count > 1) {
				...
				double avgSpeed = (probes[0].Speed + probes[1].Speed) / 2;
				double avgDistance = (probes[0].Distance + probes[1].Distance) / 2;
				long avgDuration = (probes[0].Duration + probes[1].Duration) / 2;
				double avgBearing = averageAngle(probes[0].Bearing, probes[1].Bearing);
		...
		private double averageAngle(double a, double b) {
			double anorm = normalizeAngle(a);
			double bnorm = normalizeAngle(b);

			double minAngle = Math.Min(anorm, bnorm);
			double maxAngle = Math.Max(anorm, bnorm);

			double dist1 = Math.Abs(a - b);
			double dist2 = minAngle + (360 - maxAngle);
		...
		private bool compareBearing(double baseBearing, double bearing, double range, bool allowReverse) {
			...
			double normalizedBase = normalizeAngle(baseBearing);
			double normalizedBearing = normalizeAngle(bearing);

			double min = normalizeAngle(normalizedBase - range);
			double max = normalizeAngle(normalizedBase + range);
